...
inline void to_json(nlohmann::json& convJson, const ConvolutionWrwAttributes& conv)
{
    auto& inputs = convJson["inputs"] = {};

    inputs["x_tensor_uid"] = conv.x_tensor_uid();
    inputs["dy_tensor_uid"] = conv.dy_tensor_uid();

    convJson["outputs"]["dw_tensor_uid"] = conv.dw_tensor_uid();
...
{
template <>
inline auto to<data_objects::ConvolutionWrwAttributes>(flatbuffers::FlatBufferBuilder& builder,
                                                       const nlohmann::json& entry)
{
    ...
    return data_objects::CreateConvolutionWrwAttributesDirect(
        builder,
        inputs.at("x_tensor_uid").get<int64_t>(),
        inputs.at("dy_tensor_uid").get<int64_t>(),
        outputs.at("dw_tensor_uid").get<int64_t>(),
        &prePadding,
        &postPadding,
        &stride,
        &dilation,
